...
def create_bundle(index_dir, output_path, name, description, center_lat, center_lon,
                  ...
                  creator="anonymous"):
    """Package a local Area index into a .area bundle.
    
    Only includes entries within the specified radius of the center point.
    
    Args:
        index_dir: Directory containing area_descriptors.npy, metadata.npz, etc.
        output_path: Where to save the .area file
    ...
    tmp_dir = tempfile.mkdtemp(prefix="area_bundle_")
    try:
...
def extract_bundle(bundle_path, index_dir):
    """Extract a .area bundle into Area's index directory.
    
    Args:
        bundle_path: Path to .area file
    ...
    with zipfile.ZipFile(bundle_path, 'r') as zf:
        ...
        file_mapping = {
            "descriptors.npy": "area_descriptors.npy",
            "metadata.npz": "metadata.npz",
            "pca_model.pkl": "area_pca.pkl",
...
def _make_repo_id(city, radius_km, org=None):
    ...
    name = f"area-{slug}-{int(radius_km)}km"
...
Pre-computed Area index for **Area** geolocation.
...
```python
from area_hub import AreaHub

hub = AreaHub()
hub.download("{manifest['name'].lower().replace(' ', '-')}", output_dir="./area_data/index")
# Now open Area and search!
```

Or download manually and use **Import Index** in the Area GUI.
...
class AreaHub:
    """Client for sharing Area indexes via Hugging Face Hub.
    ...
    def list_indexes(self):
        """List all available Area indexes on the hub.
        
        Returns:
            List of dicts with index metadata
        """
        print(f"[HUB] Searching globally for Area indexes...")
        try:
            ...
            seen_ids = set()
            datasets = []
            for ds in datasets_global + datasets_legacy + datasets_mine:
        ...
        for ds in datasets:
            try:
                ...
                manifest["is_official"] = (author == "area-hub")
    ...
    def download(self, repo_name, output_dir, progress_callback=None):
        ...
        Args:
            repo_name: Either full repo ID ("area-community/moscow-10km")
                       or just the short name ("moscow-10km")
            output_dir: Area COMPACT_INDEX_DIR to install into
    ...
    def upload(self, index_dir, city, radius_km, center_lat, center_lon,
        ...
        base_tags = ["area-geolocation", "geolocation", f"area-{city.lower().replace(' ', '-')}"]
        if tags:
        ...
        bundle_path = os.path.join(tempfile.gettempdir(), f"{city.lower()}_{int(radius_km)}km.area")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Area Community Index Hub")
    ...
    exp_cmd = sub.add_parser("export", help="Export index as .area file (no upload)")
    exp_cmd.add_argument("--index-dir", required=True, help="Path to index directory")
    exp_cmd.add_argument("--output", "-o", required=True, help="Output .area file path")
    ...
    imp_cmd = sub.add_parser("import", help="Import a .area file")
    imp_cmd.add_argument("bundle", help="Path to .area file")
    ...
    if args.command == "list":
        ...
        create_bundle(
            ...
            description=f"Area index for {args.city}",
            center_lat=args.lat,
            center_lon=args.lon,
            radius_km=args.radius,
        )
